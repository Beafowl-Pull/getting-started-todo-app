...
describe('PATCH /api/me', () => {
  ...
  it('should return 200 when updating name only (no password needed)', async (): Promise<void> => {
      const updatedUser = { ...sampleUser, name: 'New Name' };
      mockDb.findUserById.mockResolvedValueOnce(sampleUser).mockResolvedValueOnce(updatedUser);
  ...
  it('should return 200 when updating email with correct currentPassword', async (): Promise<void> => {
      const userWithHash = { ...sampleUser, password: hashedPassword };
      const updatedUser = { ...sampleUser, email: 'new@example.com' };
      mockDb.findUserById.mockResolvedValueOnce(userWithHash).mockResolvedValueOnce(updatedUser);
  ...
  it('should return 403 if currentPassword is incorrect when changing email', async (): Promise<void> => {
      const userWithHash = { ...sampleUser, password: hashedPassword };
  ...
  it('should return 400 if currentPassword is missing when changing email', async (): Promise<void> => {
      const res = await request(app).patch('/api/me').send({ email: 'new@example.com' });

      expect(res.status).toBe(400);
  });

  it('should return 400 if currentPassword is missing when changing password', async (): Promise<void> => {
      const res = await request(app).patch('/api/me').send({ newPassword: 'newpassword123' });
