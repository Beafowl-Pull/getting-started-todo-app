...
jest.mock('../src/routes/getGreeting', () =>
    jest.fn((_req: unknown, res: any) => res.status(200).json({ greeting: 'Hello world!' })),
);
...
describe('src/index.ts', () => {
    describe('Routes', () => {
        it('GET /api/greeting should be registered', async (): Promise<void> => {
            const res = await request(app).get('/api/greeting');
            expect(res.status).toBe(200);
        });

        it('POST /api/auth/register should be registered', async (): Promise<void> => {
            const res = await request(app).post('/api/auth/register').send({});
            expect(res.status).toBe(201);
        });

        it('POST /api/auth/login should be registered', async (): Promise<void> => {
            const res = await request(app).post('/api/auth/login').send({});
            expect(res.status).toBe(200);
        });

        it('GET /api/me should be registered', async (): Promise<void> => {
            const res = await request(app).get('/api/me');
            expect(res.status).toBe(200);
        });

        it('PATCH /api/me should be registered', async (): Promise<void> => {
            const res = await request(app).patch('/api/me').send({});
            expect(res.status).toBe(200);
        });

        it('GET /api/me/export should be registered', async (): Promise<void> => {
            const res = await request(app).get('/api/me/export');
            expect(res.status).toBe(200);
        });

        it('DELETE /api/me should be registered', async (): Promise<void> => {
            const res = await request(app).delete('/api/me');
            expect(res.status).toBe(204);
        });

        it('GET /api/items should be registered', async (): Promise<void> => {
            const res = await request(app).get('/api/items');
            expect(res.status).toBe(200);
        });

        it('POST /api/items should be registered', async (): Promise<void> => {
            const res = await request(app).post('/api/items').send({ name: 'Test' });
            expect(res.status).toBe(201);
        });

        it('PUT /api/items/:id should be registered', async (): Promise<void> => {
            const res = await request(app).put('/api/items/abc-123').send({ name: 'Updated' });
            expect(res.status).toBe(200);
        });

        it('DELETE /api/items/:id should be registered', async (): Promise<void> => {
            const res = await request(app).delete('/api/items/abc-123');
            expect(res.status).toBe(204);
        });

        it('should return 404 for unknown routes', async (): Promise<void> => {
    ...
    describe('Middleware', () => {
        it('should parse JSON bodies', async (): Promise<void> => {
        ...
        it('should return 404 for unknown static files', async (): Promise<void> => {
    ...
    describe('Graceful shutdown', () => {
        ...
        it('should call db.teardown and process.exit(0) on success', async (): Promise<void> => {
        ...
        it('should still call process.exit(0) even if teardown throws', async (): Promise<void> => {
