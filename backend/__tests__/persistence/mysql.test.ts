...
describe('MySQL persistence', () => {
  ...
  describe('init()', () => {
    it('should create the pool and the tables on success', async () => {
      // init uses a query() helper that always calls pool.query(sql, values, callback)
      mockQuery
        .mockImplementationOnce((_sql: string, _v: unknown[], cb: Function) => cb(null))                           // CREATE TABLE todo_items
        .mockImplementationOnce((_sql: string, _v: unknown[], cb: Function) => cb(null, [{ count: 1 }]))           // INFORMATION_SCHEMA check (column exists)
        .mockImplementationOnce((_sql: string, _v: unknown[], cb: Function) => cb(null))                           // CREATE INDEX
        .mockImplementationOnce((_sql: string, _v: unknown[], cb: Function) => cb(null));                          // CREATE TABLE users
    ...
    it('should run ALTER TABLE when user_id column is missing', async () => {
      mockQuery
        .mockImplementationOnce((_sql: string, _v: unknown[], cb: Function) => cb(null))                           // CREATE TABLE todo_items
        .mockImplementationOnce((_sql: string, _v: unknown[], cb: Function) => cb(null, [{ count: 0 }]))           // INFORMATION_SCHEMA check (column missing)
        .mockImplementationOnce((_sql: string, _v: unknown[], cb: Function) => cb(null))                           // ALTER TABLE ADD COLUMN
        .mockImplementationOnce((_sql: string, _v: unknown[], cb: Function) => cb(null))                           // CREATE INDEX
        .mockImplementationOnce((_sql: string, _v: unknown[], cb: Function) => cb(null));                          // CREATE TABLE users
  ...
  describe('getItems()', () => {
    it('should return all items for userId with completed as boolean', async () => {
  ...
  describe('deleteUser()', () => {
    it('should delete todos and user', async () => {
      mockQuery
        .mockImplementationOnce((_sql: string, _params: unknown[], cb: Function) => cb(null)) // DELETE todos
        .mockImplementationOnce((_sql: string, _params: unknown[], cb: Function) => cb(null)); // DELETE user

      await expect(db.deleteUser('user-1')).resolves.toBeUndefined();
    });

    it('should reject if deleting todos fails', async () => {
    ...
    it('should reject if deleting user fails', async () => {
      mockQuery
        .mockImplementationOnce((_sql: string, _params: unknown[], cb: Function) => cb(null)) // DELETE todos
  ...
  describe('getAllUserData()', () => {
    ...
    it('should reject if user query fails', async () => {
      mockQuery.mockImplementationOnce((_sql: string, _params: unknown[], cb: Function) =>
        cb(new Error('Query error')),
      );
